...
ALPHA_DECAY = 0.998 #decay constant for alpha
...
#define the rewards
REWARD_WIN = 300 #high bonus for finding cup
REWARD_DEATH = -100 #high penalty for getting caught by death eater
REWARD_MOVEMENT = -0.3  #low penalty for movement so harry will find cup fast
...
#q table
q_table = np.zeros((ROWS,COLUMNS,4)) #3D with first 2 dimensions as coordinates and last as number of possible actions
...
def action_fn(pos):  #action function
    if np.random.rand() < EPSILON :
        ...
        x, y = pos
        return ACTIONS[np.argmax(q_table[y][x])]
...
def train_harry(map_version):
    ...
    for episode in range(NO_OF_EPISODES):
        ...
        while running and steps <MAX_STEPS:
            ...
            action = action_fn(harry_pos) #gets the action(epsilon greedy policy)
        ...
        #to make sure harry doesn't go in walls
            ...
            q_value_old = q_table[harry_pos[1]][harry_pos[0]][ACTIONS.index(action)] #for finding the old q value
            q_value_new = np.max(q_table[next_harry_pos[1]][next_harry_pos[0]]) #gives the new q value by taking the max q value from the 4 actions
            tde = reward + GAMMA*q_value_new - q_value_old #calulating temporal difference error
            q_table[harry_pos[1]][harry_pos[0]][ACTIONS.index(action)] = q_value_old + current_alpha*tde #bellman equation
        ...
        #tracking progress every 10000 episodes
        if (episode + 1) % 5000 == 0:
