...
def best_action_fn(harry_pos, death_pos):  #function to provide the best action. As model is no longer we dont need random actions
    x,y = harry_pos
    dx,dy = death_pos
    return ACTIONS[np.argmax(q_table[y][x][dy][dx])] #action with highest q value is chosen
...
def run_episode(map_version): #for running an episode
    ...
    while running:
        action = best_action_fn(harry_pos,death_pos) #get action
