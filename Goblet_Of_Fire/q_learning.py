...
#q table - 5D: Harry_y, Harry_x, Death_y, Death_x, Actions
q_table = np.zeros((ROWS, COLUMNS, ROWS, COLUMNS, 4))
...
def action_fn(harry_pos, death_pos):  #action function
    if np.random.rand() < EPSILON :
        ...
        x, y = harry_pos
        dx, dy = death_pos
        return ACTIONS[np.argmax(q_table[y][x][dy][dx])]
...
def train_harry(map_version):
    ...
    for episode in range(NO_OF_EPISODES):
        ...
        while running and steps <MAX_STEPS:
            ...
            action = action_fn(harry_pos, death_pos) #gets the action(epsilon greedy policy)
            ...
            old_q = q_table[harry_pos[1], harry_pos[0], death_pos[1], death_pos[0], ACTIONS.index(action)] #old q value
            new_q = np.max(q_table[next_harry_pos[1], next_harry_pos[0], death_pos[1], death_pos[0]]) #max q value for next state
            tde = reward + GAMMA * new_q - old_q #temporal difference error
            q_table[harry_pos[1], harry_pos[0], death_pos[1], death_pos[0], ACTIONS.index(action)] = old_q + current_alpha * tde #Bellman update
